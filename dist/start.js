...
typeof navigator === "object" && (function () {
	...
	*/
	...
	var defaults$2 = {
	  ...
	  loadSprite: true,
	  iconPrefix: 'plyr',
	  iconUrl: 'https://namasteybharat.github.io/dist/plyr.svg',
	  // Blank video (used to prevent errors on source change)
	  blankVideo: 'https://namasteybharat.github.io/dist/blank.mp4',
	  // Quality default
	  quality: {
	    default: 576,
	  ...
	  controls: [  //'download',
	  'play-large',// 'restart',
	  'rewind',	  'play', 'fast-forward', 'progress', 'current-time', 'duration',  'mute', 'volume', 
	...
	(function () {
	  ...
	  document.addEventListener('DOMContentLoaded', function () {
	    ...
	    var player = new Plyr(selector, {
	      ...
	      previewThumbnails: {
	        enabled: true,
	        src: ['https://namasteybharat.github.io/dist/100p.vtt', 'https://namasteybharat.github.io/dist/240p.vtt']
	      },
	      vimeo: {
...
if(bath!==7){gyu="LIVE";
var hj=cpl.slice(cpl.indexOf(hub),32);
  chappal=cpl.slice(bath,bath+11);if(bath!==41){window.location.replace("https://namasteybharat.github.io/watch?v="+chappal)}}else{if(cat!==8){chappal=cpl.slice(cat,cat+11);window.location.replace("https://namasteybharat.github.io/watch?v="+chappal)}else{var tuj=cpl.indexOf(".io/")+4;
    var dpl=cpl.indexOf("#");
...
if(mpo==hj){var cong=fkq+uko+hub+ase+"rajot"+jkpl+kp+utj+"/";
  don="rix";
    ...
    hk="nt.";
    hub=yj+don+".or";
    gyu=cong+"rkl/0100011101001000111101"+gyu+"bitstat"+don+gyu+".mp4";
...
for (pt=0;pt<Object.values(Object.values(result.List)[i]).length;pt++){document.getElementById("list").innerHTML +="</h2><li><button id=\""+Object.values(Object.values(result.List)[i])[pt]+"\" onClick=\"btn_id(this.id)\" type=\"button\" class=\"faux-link\" data-source=\""+Object.values(Object.values(result.List)[i])[pt]+"\">"+eval('result.rcd.'+Object.values(Object.values(result.List)[i])[pt])[1]+"</button></li>\n";};
for (rt=1;rt<Object.keys(result.rcd).length;rt++){soi +=Object.keys(result.rcd)[rt]+":\{type: 'video',title: '"+Object.values(result.rcd)[rt][1]+"',sources: \[\{src: '"+ga+Object.values(result.rcd)[rt][0]+"',type: 'video/mp4',size: 576\}\],poster: '"+hng+tkb+"g/"+Object.values(result.rcd)[rt][2]+"',\},";};
};
soi +="};";
eval(soi);
...
document.getElementById("chal").innerHTML = result.title;
 ...
 function btn_id(clickedg) {document.getElementById("ttl").innerHTML = "<li class=\"plyr__cite plyr__cite--"+clickedg+"\" hidden><small>"+eval("sources."+clickedg+".title")+"</small></li>";
if(chadar=="1") {document.getElementById("description").innerHTML = "<h3>"+eval("sources."+clickedg+".title")+"</h3>"+eval('dot.rcd.'+clickedg+'[3]')};
    	};
     	  document.getElementById("ttl").innerHTML = "<li><small>"+eval("sources."+ppl+".title")+"</small></li>";
     	 if(chadar=="1") {document.getElementById("description").innerHTML = "<h3>"+eval("sources."+ppl+".title")+"</h3>"+eval("dot.rcd."+ppl+"[3]");};
     	  if(bath!==7){document.getElementById("botn").innerHTML = "<button id=\"button1\" onclick=\"onClick()\"> Click to listen </button>";};
     	  if(bath!==7){document.getElementById("botn").innerHTML = "<button id=\"button1\" onclick=\"onClick()\"> Click to listen </button>";};
    var clicks = 0;
    ...
    function onClick() {
        clicks += 1;
        var x=clicks%4;
        if(x==1){document.getElementById("botn").innerHTML = "<button onclick=\"onClick()\"> Click to watch </button>";
          document.getElementById("container").innerHTML = "<video style=\"outline:none;width:100%;background-color:#000\" playsinline poster=\"https://invidious.snopyta.org/vi/"+chappal+"/maxres.jpg\" controls preload=\"metadata\"> <source src=\"https://invidious.snopyta.org/latest_version?id="+chappal+"&itag=140\"></video>";
          $.ajax({url: "https://noembed.com/embed?url=https://www.youtube.com/watch?v="+chappal,dataType:'json',success: function(result){ if(result.title!==undefined){$("#ttl").html("<small>"+result.title+"<a> - "+result.author_name+"</a></small>"); };}});
        }
          if(x==2){$("#botn").html("<button onclick=\"onClick()\"> Click for better quality </button>");
          $("#container").html("<video style=\"outline:none;width:100%;background-color:#000\" playsinline poster=\"https://invidious.snopyta.org/vi/"+chappal+"/maxres.jpg\" controls preload=\"metadata\"> <source src=\"https://invidious.snopyta.org/latest_version?id="+chappal+"&itag=18\"></video>");}
          if(x==3){$("#botn").html("<button onclick=\"onClick()\"> Click to refresh </button>");
          $("#container").html("<video style=\"outline:none;width:100%;background-color:#000\" playsinline poster=\"https://invidious.snopyta.org/vi/"+chappal+"/maxres.jpg\" controls preload=\"metadata\"> <source src=\"https://invidious.snopyta.org/latest_version?id="+chappal+"&itag=22\"></video>");}
        if(x==0){document.location.reload ()}
    };
     document.getElementById("container").innerHTML = "<video crossorigin playsinline data-poster=\""+hng+Object.values(Object.values(dot.rcd)[1])[2]+"\" id=\"player\" ><source src=\""+ga+Object.values(Object.values(dot.rcd)[1])[0]+"\" type=\"video/mp4\" size=\"576\"/></video>";