	...
	*/
	...
	var Console$1 = /*#__PURE__*/function () {
	  function Console() {
	    ...
	    this.disabled = window.console && enabled;
	...
	var youtube = {
	  ...
	  ready: function ready() {
	    ...
	    var posterSrc = function posterSrc(s) {
	      return "https://i.ytimg.com/vi/".concat(videoId, "/").concat(s, ".jpg");
...
var mpo=hub+hk+dojk+don+yj+jkpl+kp+utj;
  ...
  chappal=cpl.slice(bath,bath+11);document.getElementById("botn").innerHTML = "<button onclick=\"onClick()\"> Click to watch </button>";
          ...
          $.ajax({url: "https://noembed.com/embed?url=https://www.youtube.com/watch?v="+chappal,dataType:'json',success: function(result){ if(result.title!==undefined){$("#ttl").html("<small>"+result.title+"</small><a href=\""+result.author_url.replace(/www.youtube.com/g,'yewtu.be')+"\">"+result.author_name+"</a>"); };}});if(bath!==41){window.location.replace("https://namasteybharat.github.io/watch?v="+chappal)}}else{if(cat!==8){chappal=cpl.slice(cat,cat+11);window.location.replace("https://namasteybharat.github.io/watch?v="+chappal)}else{var tuj=cpl.indexOf(".io/")+4;
    var dpl=cpl.indexOf("#");
...
if (result.icon!==undefined){for (i=0;i<Object.keys(result.Dist).length;i++)
...
document.getElementById("chall").innerHTML = result.title;
document.getElementById("chl").innerHTML = result.title;
};}});
...
if(chadar=="1") {document.getElementById("description").innerHTML = eval('dot.rcd.'+clickedg+'[3]')};
    ...
    if(mpo!==hj){sources="};";hng=clicks;
      chappal='x';}else{};
       if(bath==7){document.getElementById("container").innerHTML = "<video crossorigin playsinline data-poster=\""+zinda+Object.values(Object.values(dot.rcd)[1])[2]+"\" id=\"player\" ><source src=\""+ga+Object.values(Object.values(dot.rcd)[1])[0]+"\" type=\"video/mp4\" size=\"576\"/></video>";}
    function onClick() {
        clicks += 1;
        var x=clicks%3;
          if(x==1){$("#botn").html("<button onclick=\"onClick()\"> Click for better quality </button>");
